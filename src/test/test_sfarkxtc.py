...
try:
    ...
    raise ImportError(
        'Please first run ' + '`pip install shinerainsevenlib` to run these tests.'
    )
...
def goDir(d, actualChecksums):
    ...
    for f, short in workToDo:
        ...
        expectCountNewItems = 1 + (1 if shouldContainLicense else
                                   0) + (1 if shouldContainNotes else 0)
        ...
        if shouldContainNotes:
            ...
            assertTrue(
                files.getName(nameToCompare) in actualChecksums, expectNotes, actualChecksums
            )
